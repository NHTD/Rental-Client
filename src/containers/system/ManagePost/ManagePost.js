...
const ManagePost = () => {
  ...
  const handleFilterStatus = (status) => {
    if (+status === 1) {
      const activePost = postsOfUser?.postResponse?.filter(
        (el) => el?.status === "ACTIVE"
      );
      setPosts(activePost);
    } else if (+status === 0) {
      const expiredPost = postsOfUser?.postResponse?.filter(
        (el) => !el?.status
      );
  ...
  return (
    <div className={cx("manage-post")}>
      <div className={cx("manage-post__row")}>
        ...
        <select
          className={cx("manage-post__select")}
      ...
      <table className={cx("manage-post__table")}>
        <thead>
          <tr className={cx("manage-post__table-header")}>
            <th
              className={cx("manage-post__header", "manage-post__header-idx")}
            >
              Mã tin
            </th>
          ...
          {Array.isArray(posts) && posts.length > 0 ? (
            posts.map((el, index) => (
              <tr key={el?.id} className={cx("manage-post__table-row")}>
                <td
                  className={cx("manage-post__body", "manage-post__body-idx")}
                >
                  {index + 1}
                </td>
                <td className={cx("manage-post__body-thumbnail")}>
                  {el?.images?.map((item) => (
                    <img
                      src={item?.image}
                      alt="thumbnail"
                      className={cx("manage-post__thumbnail")}
                    />
                  ))}
                </td>
                <td
                ...
                <td className={cx("manage-post__body-action")}>
                  ...
                  <button
                    className={cx("manage-post__body-delete")}
      ...
      {isEdit && (
        <UpdatePost dataEdit={dataEdit} isEdit={isEdit} setIsEdit={setIsEdit} />
      )}
    </div>
  );
};
