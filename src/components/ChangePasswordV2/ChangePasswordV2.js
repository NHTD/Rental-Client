...
import { apiVerifyFromPassword, apiVerifyOtp } from '../../services/user'
...
const ChangePasswordV2 = () => {
    ...
    const handleChangePassword = async () => {
        ...
        if(response.status===200) {
            Swal.fire({title: "Thành công", icon: "success", width: "450px", showConfirmButton: true, timer: 2000})
            .then(() => setPayload(""))
            .then(() => navigate(`/${path.LOGIN}`))
        }else {
            Swal.fire({title: "Thất bại", text: "Thay đổi mật khẩu thành công", icon: "error", width: "450px", showConfirmButton: true, timer: 2000})
        }
    }
  ...
  return (
    <div className={cx("otp")}>
        ...
        <div className={cx("otp__content")}>
            <span className={cx("otp__desc")}>
                Vui nhập mật khẩu
            </span>

            <div className={cx("otp__inner")}>
                <label htmlFor='otp' className={cx("otp__label")}>Mật khẩu cũ</label>
                <input
                    type='text'
                    id='oldPassword'
                    name='oldPassword'
                    value={payload.oldPassword}
                    onChange={(e) => setPayload(prev => ({...prev, oldPassword: e.target.value}))}
                    className={cx("otp__input")}
                />
            </div>

            <div className={cx("otp__inner")}>
                <label htmlFor='newPassword' className={cx("otp__label")}>Mật khẩu mới</label>
                <input
                    type='text'
                    id='newPassword'
                    name='newPassword'
